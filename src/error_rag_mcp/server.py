...
def create_server() -> MCPServer:
    ...
    async def search_similar_error(error_summary: str, error_keyword: str) -> str:
        """과거 동일/유사 오류 사례와 조치 방법을 RAG 서비스에서 검색합니다.
        오류 요약으로 벡터(의미) 검색을, 오류 코드 등 핵심 키워드로 텍스트 매칭(정확 일치) 검색을
        함께 수행하여 결과를 병합·중복 제거해 반환합니다. 조치 방법 판단은 이 결과를 참고해
        호출자(AI Agent)가 직접 수행하세요.

        Args:
            error_summary: 오류 내용 요약 (300자 이내). 벡터 검색 쿼리로 사용됩니다.
            error_keyword: 오류 코드 등 핵심 식별 키워드. 텍스트 매칭 검색 쿼리로 사용됩니다.
        """
        try:
    ...
    async def register_error_resolution(
        ...
        """오류 및 조치 결과를 표준 보고서 형식으로 RAG 서비스에 등록합니다. 조치 완료 후 반드시
        호출하여, 다음에 동일/유사 오류 발생 시 재활용할 수 있는 지식을 축적하세요.

        Args:
            error_summary: 오류 내용 요약 (300자 이내). 향후 벡터/텍스트 매칭 검색 대상이 됩니다.
            error_keyword: 오류 코드 등 핵심 식별 키워드 목록 (최대 3개, 1개 이상 필수). 텍스트
                매칭 검색으로 이 사례를 찾을 수 있도록 content에 결합되어 저장됩니다.
        ...
        try:
            ...
            if not error_keyword or len(error_keyword) > MAX_ERROR_KEYWORD_COUNT:
                raise ValueError(
                    f"error_keyword는 1개 이상 {MAX_ERROR_KEYWORD_COUNT}개 이하로 지정해야 합니다."
                )
